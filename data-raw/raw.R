...
clean_weekly_npi_file <- function(path) {
  ...
  healthcare_provider_taxonomy_code|provider_license_number|provider_license_number_state_code|
  healthcare_provider_primary_taxonomy_switch|healthcare_provider_taxonomy_group
                                             ")
...
make_id <- \(x) gsub("-", "_", tools::file_path_sans_ext(basename(x)))

npidata_paths <- dplyr::filter(get_pin("nber_weekly_info")$unzipped, file == "npidata_pfile")$path


clean_weekly_npi_file(npidata_paths[1])
